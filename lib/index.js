...
function serializeSession(item) {
  ...
  if (_isObject(item)) {
    Object.keys(item).forEach(function (key) {
      if (key.match(/\|/)) {
        (0, _assert2.default)(false, 'Top level name "' + key + '" may not contain pipes');
      } else {
        s = '' + s + key + '|' + serialize(item[key]);
      }
    });
  } else {
    (0, _assert2.default)(false, 'Unable to serialize sessions with top level types other than Object');
  }
  return s;
}

module.exports = { serialize: serialize, unserialize: unserialize, serializeSession: serializeSession };